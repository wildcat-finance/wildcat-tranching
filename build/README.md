...
# everything (local + mainnet fork): 24 tests
forge test

# local only (unit + fuzz + invariants)
forge test --no-match-path test/Fork.t.sol

# mainnet fork only (uses https://eth-main.hinterlight.net)
...
redemption through the real withdrawal queue.
...
redemption queue. Demo links: `index.html?demo=loss`, `index.html?demo=seniorfirst`.
